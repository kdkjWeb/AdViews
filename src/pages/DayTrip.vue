    ...
    <div class="container">
      ...
      <div class="handle-box">
        <el-row>
          ...
          <el-col :span="6">
            <div class="addUser">
              <el-button size="mini" type="primary" class="handle-del mr10" @click="handleAddUser">新增一日游</el-button>
            </div>
          </el-col>
        </el-row>
      </div>
    ...
    <el-dialog
      ...
      <div>
        <el-form ref="form" :model="form" :rules="rules" label-width="100px" size="mini">
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="供应商" prop="merchatName">
                <el-input v-model="form.merchatName" clearable></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
  ...
  export default {
    ...
    data(){
      return{
        ...
        tableList: [   //表格的头部配置
          ...
          {prop: 'merchatName', label: '供应商', width: '', align: ''},
    ...
    methods: {
      ...
      handleComfirm(formName){
        var url = this.id ? '/onedayTour/updateModel' : '/onedayTour/insertModel';

        if(this.id){   //id存在  修改一日游
          this.$put(url,{
            destination: this.form.destination,  //目的地
            title: this.form.title,   //密码
            merchatName: this.form.merchatName,   //供应商
            price: this.form.price,   //角色
      ...
      //获取表格数据
      getDataList(pageNum){
        this.$get('/onedayTour/queryByRecord',{
      ...
      handleEdit(row){
        ...
        this.dialogTitle = '编辑一日游';

        this.id = row.id;

        //获取用户详情
        this.$get('/onedayTour/selectById',{
          id: row.id
        }).then(res=>{
          if(res.code == 0){
            this.dialogVisible = true;
      ...
      //删除表格某一行
      handleDelete(row){
        this.$confirm('此操作将永久删除该一日游, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$delete('/onedayTour/deleteById',{
            id: row.id
          }).then(res=>{
            if(res.code == 0){
              this.$message({
